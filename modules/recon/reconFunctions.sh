...
runSearch(){
	...
	# Using interlace and ffuf CHECKKKKKKKKKK
	sudo interlace -tL "$SUBS"/hosts.txt -threads 5 --silent -c ""$GOBIN"/ffuf -u _target_/FUZZ -w "$BASE"/nullbot/modules/recon/wordlists/fuzz.txt -fc 404,400,500 -or -o ffuftest.txt -v 2>/dev/null | grep URL | cut -d '|' -f 3 | "$GOBIN"/anew -q "$DIRSEARCH"/interlace.txt"
...
. ${BASE}/nullbot/modules/recon/configs/tokens